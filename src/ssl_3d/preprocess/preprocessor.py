...
from ssl_3d.preprocess.nifti_blosc2_transformer import Blosc2Compressor
import os 
import numpy as np
from ssl_3d.preprocess.potential_centers import process_one_case

import nibabel as nib

from nucli_train.data_management.data_creation import save_blosc
...
class PreprocessorBlosc2: 
    ...
    def __init__(
        ...
        if self.transformation is not None:
            self.dataset_type = "blosc2_dataset_transform"
        else: self.dataset_type = "blosc2_dataset"
        ...
        if shuffle_pick: 
            self.chosen_indices = np.random.permutation(np.arange(len(self.list_potential_input_nifti)))[:self.number_of_cases]
        else: 
        ...
        self.nucli_train_path = os.path.join(self.dataset_path,"nucli_train" ,self.exp_name, self.dataset_name)
        ...
        for center, tracers in self.input_train_dico.items():
            for tracer, nifti_filenames in tracers.items():
                specific_dir = os.path.join(self.b2ndrootdir_train, self.dataset_name, center, tracer, input_name)
        ...
        for center, tracers in self.input_val_dico.items():
            for tracer, nifti_filenames in tracers.items():
                specific_dir = os.path.join(self.b2ndrootdir_val, self.dataset_name, center, tracer, input_name)
                if not os.path.exists(specific_dir):
                    os.makedirs(specific_dir)
                else: 
                    shutil.rmtree(specific_dir)
        ...
        if self.nifti_target_rootdir is not None:
            for center, tracers in self.target_train_dico.items():
                for tracer, nifti_filenames in tracers.items():
                    specific_dir = os.path.join(self.b2ndrootdir_train, self.dataset_name, center, tracer, target_name)
                    if not os.path.exists(specific_dir):
                        os.makedirs(specific_dir)
                    else: 
                        shutil.rmtree(specific_dir)
            ...
            for center, tracers in self.target_val_dico.items():
                for tracer, nifti_filenames in tracers.items():
                    specific_dir = os.path.join(self.b2ndrootdir_val, self.dataset_name, center, tracer, target_name)
                    if not os.path.exists(specific_dir):
                        os.makedirs(specific_dir)
                    else: 
                        shutil.rmtree(specific_dir)
        ...
        self.compress_and_coords(self.input_train_dico, self.b2ndrootdir_train, modality=input_name)
        self.compress_and_coords(self.input_val_dico, self.b2ndrootdir_val, modality=input_name)

        if self.nifti_target_rootdir is not None:
            self.compress_and_coords(self.target_train_dico, self.b2ndrootdir_train, modality=target_name)
            self.compress_and_coords(self.target_val_dico, self.b2ndrootdir_val, modality=target_name)
    ...
    def load_nifti(self, path): 
        ...
        if self.resample:
            target_spacing = np.array(self.spacing)
            print(f"Resampling volume from {current_spacing} to {target_spacing} with initial shape {volume.shape}")
            volume = resample(volume, current_spacing, target_spacing)
            print(f"Resampled volume shape: {volume.shape}")
        return volume
        ...
        for center, tracers in dico.items():
            for tracer, nifti_filenames in tracers.items():
                for nifti_filename in nifti_filenames:
                    ...
                    specific_file_meta_data = specific_file_path_b2nd + ".meta.yaml"

                    volume = self.load_nifti(nifti_path)

                    shape = volume.shape
                    ## Resample the volume if target_spacing is provided

                    #self.blosc2_compressor.save_case(volume, self.patch_size, specific_file_path_b2nd)
                    result = save_blosc(specific_file_path_b2nd, volume, np.array(self.patch_size))
                    ...
                    # Save the metadata
                    with open(specific_file_meta_data, 'w') as f:
    ...
    def write_main_yaml(self, main_config_path: str, train_config_path: str, val_config_path: str):
        ...
        'transformation_args': self.transformation_args,
...
if __name__ == "__main__":
    ...
    preprocessor = PreprocessorBlosc2(
        ...
        dataset_path="/home/ec2-user/data/autopet_2024",
